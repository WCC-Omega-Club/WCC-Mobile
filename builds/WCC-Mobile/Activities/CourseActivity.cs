...
namespace WCCMobile
{
    ...
    public class Course
    {
        ...
        {
            CourseDay right;
            foreach(string itr in a.DAYS.Keys)
            {
                if (b.days.TryGetValue(itr, out right))
                {
                    CourseDay left = a.days[itr];

                    if (left.START_TIME == right.START_TIME || left.END_TIME == right.END_TIME)
                        return true;
                    if((left.START_TIME < right.START_TIME) && (left.END_TIME > right.START_TIME))
                        return true;
                    if ((left.START_TIME > right.START_TIME) && (right.END_TIME < left.START_TIME))
                        return true;
                }
